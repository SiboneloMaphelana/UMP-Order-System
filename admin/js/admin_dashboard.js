    ...
    function addOrderToTable(order) {
        ...
        orderRow.innerHTML = `
            ...
            <td>
                ...
                <button class="btn btn-primary btn-sm" onclick="showUpdateStatusModal(${order.id})">Update</button>
            </td>
        `;
        ...
        fetch(`controllers/fetch_order_details.php?order_id=${orderId}`)
            ...
            .then(order => {
                ...
                modalBody.innerHTML = `
                    ...
                    <ul>
                        ${order.items.map(item => `<li>${item.name} - R${item.price} x ${item.quantity}</li>`).join('')}
                    </ul>
                `;
